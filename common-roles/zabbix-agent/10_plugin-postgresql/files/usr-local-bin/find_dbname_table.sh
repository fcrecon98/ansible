...
dbname_list=$(/usr/local/postgres/bin/psql -h $PGHOST -p $PGPORT -U $PGROLE -d $PGDATABASE -t -c "${GETDB}" 2>&1)
if [ $? -ne 0 ]; then
	echo "$dbname_list"
	exit
fi
...
for dbname in $dbname_list; do
	tablename_list=$(/usr/local/postgres/bin/psql -h $PGHOST -p $PGPORT -U $PGROLE -d ${dbname# } -t -c "${GETTABLE}" 2>&1)
	if [ $? -ne 0 ]; then
		echo "$tablename_list"
		exit
	fi
